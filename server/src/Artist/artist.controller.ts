...
export class ArtistController {
    ...
    async uploadmusic(
        ...
        @Request() req, 
        @UploadedFiles() files: { mp3: Express.Multer.File[], image: Express.Multer.File[] }
    ): Promise<any> {
        console.log("hii")
        ...
        const savedmusic = await this.artistservice.uploadmusic(req.user, Audiodto, files);

        return {
            meassge: "music has been uploaded successfully!", mp3: savedmusic
        }
    }
    ...
    async updateplaylistname(
        ...
        return await this.artistservice.updateplaylistname(req.user, dto.artist_playlist_id, dto.changed_name);
    }
    @UseGuards(JwtAuthGuard)
    @Post("addmusic")
