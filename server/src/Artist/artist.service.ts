...
export class ArtistService {
    ...
    async uploadmusic(user: any, dto: songdto, files: { mp3: Express.Multer.File[], image: Express.Multer.File[] },) {
        try {
            ...
            throw new HttpException("Failed to upload music", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async createartistplaylist(user: any, name: string): Promise<any> {
        try {
            ...
            if(!artist_Playlist) {
                throw new HttpException("error to create playlist!", HttpStatus.NOT_ACCEPTABLE);
            }
    ...
    async updateplaylistname(user: any , aritst_playlist_id: number, changed_name: string) {
        try {
            const playlist = await this.prisma.artist_Playlist.findUnique({
                where: {
