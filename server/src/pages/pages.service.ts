...
export class pagesservice {
  ...
  async searchbysong(query: string) {
    try {
      ...
      throw new HttpException(
        error.meassage || 'failed to serach song!',
  ...
  async searchbyplaylist(query: string) {
    try {
      ...
      throw new HttpException(
        error || 'failed to search songbyplaylist!',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async detailsLikedSong(user: any) {
    try {
      ...
      throw new HttpException(
        error.message || 'Failed to get liked song details',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async addMusicToLikedSongs(user: any, musicId: number) {
    try {
      ...
      throw new HttpException(
        error.message || 'Failed to add music to liked songs',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  private async updateLikedSongs(
    likedSongsId: number | undefined,
    musicId: number,
  ) {
    if (!likedSongsId) {
    ...
    if (existingLikedSongs.includes(musicId.toString())) {
      throw new HttpException(
        'Music already exists in liked songs!',
        HttpStatus.CONFLICT,
      );
    }
  ...
  async deletemusictolikedsongs(user: any, musicId: number) {
    try {
      ...
      if (findUser) {
        // User found, update the liked songs for the user
        return this.deleteMusicFromLikedSongs(
          findUser.Liked_songs[0]?.id,
          musicId,
        );
      }
  ...
  async deleteMusicFromLikedSongs(
    likedSongsId: number | undefined,
    musicId: number,
  ) {
    if (!likedSongsId) {
    ...
    if (musicIndex === -1) {
      throw new HttpException(
        'Music not found in liked songs!',
        HttpStatus.NOT_FOUND,
      );
    }
