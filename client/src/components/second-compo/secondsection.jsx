  ...
  useEffect(() => {
    async function getVendorData() {
      try {
        ...
        if (error) toast.error("Error fetching vendor data! ");
      }
    }
    getVendorData();
  }, []);
  ...
  return (
    <>
      {isLoggedIn === false ? (
        <div className="secondsection">
          ...
          <div className="secondsection-2">
            <div className="secondsection-2-card">
              ...
              <p>title</p>
              <p style={{ fontSize: "smaller", color: "#a7a7a7" }}>
                description
              </p>
            </div>
            <div className="secondsection-2-card">
        ...
        <div className="secondsection">
          <div className="secondsection-1">
            {vendordata ? <h3>Made For {vendordata.name}</h3> : null}
            <h3 style={{ color: "gray" }}>show all</h3>
          </div>
          <div className="secondsection-2">
            {allartistdata && allartistdata.length > 0
              ? allartistdata.map((playlist) => (
                  <div
                    className="secondsection-2-card"
                    key={playlist.id}
                    onClick={() => {
                      navigate(`/artist/${playlist.id}`);
                    }}
                  >
                    <div>
                      <img
                        src={`http://localhost:3333/uploads/artist_photos/${playlist.album_photo}`}
                        alt="liked_song"
                      />
                    </div>
                    <p>{playlist.name}</p>
